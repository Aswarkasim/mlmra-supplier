                ...
                <div class="col">
                    <div class="card">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-md">
                                    ...
                                    @foreach($commisions as $commision)
                                        <tr>
                                            <td>{{ $commision->transaction->name }}</td>
                                            <td>{{ $commision->user->username }}</td>
                                            <td>{{ $commision->reseller->username }}</td>
                                            <td>{{ $commision->commision_total }}</td>
                                            <td>{{ $commision->status ?: 'PAID' }}</td>
                                            <td>
                                                <a href="" class="btn btn-primary">Konfirmasi</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    @forelse($commisions as $commision)
                                    @empty
                                        <tr>
                                            <td colspan="10" class="text-center mt-2"><b>No Komisi</b></td>
                                        </tr>
                                    @endforelse
                                </table>
                            </div>
                        ...
                        <div class="paginate float-right mt-3 ml-3">
                            <p class="ml-3"><b>Jumlah Data : {{ $commisions->total() }}</b></p>
                            {{ $commisions->links() }}
                        </div>
                    </div>
                </div>
